...
export function update(recursion = true) {
    setTimeout(function () {
        ...
        for (let i = 0; i < buttons.length; i++) {
            (<HTMLElement>buttons.item(i)).onclick = function () {
                if ((<HTMLElement>buttons.item(i)).innerText == "Fertig") {
                    query("cuisine.update_status", `${buttons.item(i).id.split("_")[2]};1`);
                } else {
                    query("cuisine.update_status", `${buttons.item(i).id.split("_")[2]};2`);
                }
                (<HTMLElement>buttons.item(i)).innerText = "Bitte warten...";
            };
        }
        if (recursion) {
            update();
